...
const OrientationFilter = ({ onChange, initialValues = [] }) => {
    ...
    useEffect(() => {
        const fetchOrientationOptions = async () => {
            try {
                setLoading(true);
                const data = await getListValues('orientacion');
                setOrientationOptions(data);
                setError(null);
            } catch (err) {
                console.error('Error al cargar opciones de orientación:', err);
                setError('No se pudieron cargar las opciones de orientación');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="p-4 pb-16">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-white font-semibold text-lg">Orientación</h3>
                <button
                    onClick={handleReset}
